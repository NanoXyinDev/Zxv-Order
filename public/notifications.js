(function(){
  ...
  function render(meta){
    ...
    card.innerHTML=`<div class="otp-order-head"><div><span class="eyebrow">ORDER OTP</span><h3>${text(meta.id)}</h3></div><span class="otp-country">${country}</span></div>
      ...
      <span>${meta.done&&meta.otp?`Kode: <b class=\"otp-code\">${text(meta.otp)}</b>`:canCancel?'Sudah lebih dari 3 menit — dapat dibatalkan':`Tombol cancel tersedia dalam ${mins}:${String(secs).padStart(2,'0')}`}</span></div>
  ...
  async function poll(orderId){
    ...
    try{
      ...
      if(isDone(data)){
        ...
        const all=load();delete all[orderId];save(all);render(meta);notify(orderId);setTimeout(()=>{delete meta.otp;state.orders.set(orderId,meta)},30000);
  ...
  function start(orderId,meta){
    ...
    if(!state.timer)state.timer=setInterval(()=>state.orders.forEach((_,oid)=>poll(oid)),10000);
